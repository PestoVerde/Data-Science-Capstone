...
per.cent <- 0.25 #Size of samples to meet limit of shiny.io
...
#Here we read files and take part of it
...
#Here we combine texts and remove used files
parole <- c(blogs, news, twitter)
rm(blogs, news, twitter)

#Here we split text to train and test sets
...
#Here we clean our sets...
train <- clean_text(train, "profanity.txt")
test <- clean_text(test, "profanity.txt")

#...and write them to disc in order to avoid above steps in the future
...
one.matrix <- TermDocumentMatrix(corpus,
                                 control = list(tokenize = OnegramTokenizer,
                                                wordLengths=c(1,Inf)))

onegram <- converse.TDM(one.matrix)

write.csv(onegram,"onegram.csv")                      #Write all words
rm(onegram, one.matrix)
...
#Creating part for calculations
...
#Building fof (frequency of frequency) tables to calculate coefficients

w3 <- read.csv("trigram.csv", stringsAsFactors = F)
w2 <- read.csv("bigram.csv", stringsAsFactors = F)
w1 <- read.csv("onegram.csv", stringsAsFactors = F)
...
save(w1, w2, w3, d, file = 'profezia.RData')